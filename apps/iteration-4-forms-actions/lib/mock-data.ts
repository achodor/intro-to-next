...
export function getReturnById(id: string): Return | undefined {
	return mockReturns.find((returnItem) => returnItem.id === id);
}
...
export function updateReturn(
	...
	const index = mockReturns.findIndex((returnItem) => returnItem.id === id);
	if (index === -1) return null;

	mockReturns[index] = { ...mockReturns[index], ...updates };
